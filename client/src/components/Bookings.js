    ...
    },
    {
      ...
      render: (_, booking) => {
        if (booking.status === "cancelled") {
          return (
            <button disabled className="nodelete">
              Delete
            </button>
          );
        } else {
    ...
    (async () => {
      try {
        ...
        const confirmedBookings = data.filter(
          (bookings) => bookings.status === "booked"
        );
        ...
        <Tabs defaultActiveKey="1" size="large">
          <TabPane tab="Confirmed" key="1">
            <Table
              ...
              footer={() => (
                <div className="footer-number">{`Total ${bookings.length} items`}</div>
              )}
            />
          </TabPane>
          ...
          <TabPane tab="Cancelled" key="2">
            {cancelledBookings.filter(
              ...
              <Table
                ...
                footer={() => (
                  <div className="footer-number">{`Total ${cancelledBookings.length} items`}</div>
                )}
                pagination={{ pageSize: 15 }}
              />
            ) : (
